...
func NewMemoryDevicePlugin(resourceName string, manager *manager.DeviceManager, socket string) DevicePlugin {
...
func (m *MemoryDevicePlugin) Name() string {
...
func (m *MemoryDevicePlugin) ListAndWatch(_ *pluginapi.Empty, s pluginapi.DevicePlugin_ListAndWatchServer) error {
...
func (m *MemoryDevicePlugin) GetPreferredAllocation(context.Context, *pluginapi.PreferredAllocationRequest) (*pluginapi.PreferredAllocationResponse, error) {
...
func (m *MemoryDevicePlugin) Devices() []*pluginapi.Device {
	var devices []*pluginapi.Device
	for _, gpuDevice := range m.manager.GetDevices() {
		if gpuDevice.Mig { // skip mig device
			continue
		}
		config := m.manager.GetNodeConfig()
