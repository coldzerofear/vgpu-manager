...
func Test_NodePriority(t *testing.T) {
	...
	start := time.Now()
	NewNodeBinpackPriority(UniformProfile, util.NoneTopology, 0).Sort(nodeInfoList)
	since := time.Since(start)
	fmt.Printf("call NewNodeBinpackPriority took %d Milliseconds\n", since.Milliseconds())
	start = time.Now()
	NewNodeSpreadPriority(UniformProfile, util.NoneTopology, 0).Sort(nodeInfoList)
	since = time.Since(start)
...
func Test_NodeSorting(t *testing.T) {
	...
	NewNodeBinpackPriority(UniformProfile, util.LinkTopology, 2).Sort(nodes)
	...
	NewNodeSpreadPriority(UniformProfile, util.LinkTopology, 2).Sort(nodes)
	...
	NewNodeSpreadPriority(UniformProfile, util.NoneTopology, 0).Sort(nodes)
