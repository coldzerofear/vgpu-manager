...
func TestGetDeviceRequestsForPodClaim_LiveFallbackAndMutationCache(t *testing.T) {
	...
	reader := NewClaimRequestReader(nil, liveClient, newIndexer(), newIndexer(), time.Minute)
...
func TestGetDeviceRequestsForPodClaim_TemplatePath(t *testing.T) {
	...
	reader := NewClaimRequestReader(cachedClient, nil, newIndexer(), newIndexer(), time.Minute)
...
func TestGetDeviceRequestsForPodClaim_NotFoundWhenNoClients(t *testing.T) {
	ctx := context.Background()
	reader := NewClaimRequestReader(nil, nil, newIndexer(), newIndexer(), time.Minute)
