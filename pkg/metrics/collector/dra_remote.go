...
var _ claimresolve.Reader = &draGPUCollector{}

func (c draGPUCollector) GetPod(_ context.Context, key client.ObjectKey, obj *corev1.Pod) error {
...
func (c draGPUCollector) GetResourceClaim(_ context.Context, key client.ObjectKey, obj *v1.ResourceClaim) error {
	claim, err := c.claimLister.ResourceClaims(key.Namespace).Get(key.Name)
	if err != nil {
		return err
	}
...
func (c draGPUCollector) remoteConsumerPods(devInfoNameMap map[string]*DRADeviceInfo) map[types.UID]*corev1.Pod {
	...
	for _, claim := range claims {
		...
		for _, ref := range claim.Status.ReservedFor {
			...
			if err = c.GetPod(ctx, key, &pod); err != nil || (ref.UID != "" && pod.UID != ref.UID) {
				continue
			}
...
func (c draGPUCollector) remoteSessionPIDs(alloc draContainerAlloc, partitionCache map[types.UID]*claimresolve.PartitionInfo) ([]uint32, []string) {
	...
	for _, ref := range alloc.claims {
		...
		if !ok {
			...
			resolved, err := claimresolve.ResolveClaimVGPUPartitionsFromAllocatedRequests(ctx, c, ref.claim, allocated)
			if err != nil {
