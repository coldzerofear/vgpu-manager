...
func newPartTestGpu(maxCapacities PartCapacityMap, memSliceCount int) *GpuDeviceInfo {
	gpu := newTestGpuInfo(newScalarNumaNodeAttribute(0))
	gpu.Memory = nvml.Memory{Total: uint64(4096)}
...
func newPartTestMigSpec(parent *GpuDeviceInfo, start, size uint32) *MigSpec {
	return &MigSpec{
		Parent:  parent,
