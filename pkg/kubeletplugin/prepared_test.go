...
func newPreparedGpu(deviceName, uuid string) PreparedDevice {
	return PreparedDevice{
		Gpu: &PreparedGpuDevice{
			Info:   &GpuDeviceInfo{GpuInfo: &nvidia.GpuInfo{UUID: uuid}},
...
func TestPreparedDeviceGroupGetDevices(t *testing.T) {
	...
	tests := map[string]struct {
		...
		"converts every kind and keeps list order": {
			group: PreparedDeviceGroup{
				Devices: PreparedDeviceList{
					{Gpu: &PreparedGpuDevice{Device: &gpu}},
					...
					Allocation: &resourceapi.AllocationResult{
						Devices: resourceapi.DeviceAllocationResult{
							Results: []resourceapi.DeviceRequestAllocationResult{
								...
								{Driver: util.DRADriverName, Device: "gpu-1", AdminAccess: ptr.To(true)},
								{Driver: util.DRADriverName, Device: "gpu-2", AdminAccess: ptr.To(false)},
							},
						},
					},
				},
