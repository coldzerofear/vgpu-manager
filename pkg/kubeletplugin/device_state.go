	...
	for _, result := range claim.Status.Allocation.Devices.Results {
		...
		for _, c := range slices.Backward(configs) {
			if slices.Contains(c.Requests, result.Request) {
				if _, ok := c.Config.(*configapi.GpuConfig); ok && device.Type() != GpuDeviceType && device.Type() != VGpuDeviceType {
		...
		for _, result := range results {
			...
			case VGpuDeviceType:
				preparedDevice.VGpu = &PreparedVGpuDevice{
					Info:   s.allocatable[result.Device].VGpu,
					Device: device,
				}
			case GpuDeviceType:
...
func (s *DeviceState) unprepareDevices(ctx context.Context, claimUID string, devices PreparedDevices) error {
	for _, group := range devices {
		...
		if featuregates.Enabled(featuregates.VGPUSupport) {
			err := s.vgpuManager.Cleanup(claimUID, group.Devices.VGpus())
			if err != nil {
...
func (s *DeviceState) applySharingConfig(ctx context.Context, config configapi.Sharing, claim *resourceapi.ResourceClaim, results []*resourceapi.DeviceRequestAllocationResult) (*DeviceConfigState, error) {
	...
	if featuregates.Enabled(featuregates.VGPUSupport) {
		configState.containerEdits = s.vgpuManager.GetCDIContainerEdits(claim, allocatableDevices)
	}
