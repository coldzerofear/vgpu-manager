...
func TestPrependPathListEnvvar(t *testing.T) {
	...
	for description, tc := range tests {
		t.Run(description, func(t *testing.T) {
			...
			require.Equal(t, tc.expected, nvidia.PrependPathListEnvvar(envvar, tc.prepend...))
		})
	}
}
...
func TestSetOrOverrideEnvvar(t *testing.T) {
	...
	for description, tc := range tests {
		t.Run(description, func(t *testing.T) {
			require.Equal(t, tc.expected, nvidia.SetOrOverrideEnvvar(tc.envvars, tc.key, tc.value))
		})
	}
}
...
func TestDeviceLibSetTimeSlice(t *testing.T) {
	t.Run("one invocation per UUID", func(t *testing.T) {
		t.Setenv("LD_PRELOAD", "")
		smi := newFakeSMI(t, 0)
		l := nvidia.DeviceLib{NvidiaSMIPath: smi.path, DriverLibraryPath: smi.libPath}
