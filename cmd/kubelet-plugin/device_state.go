...
import (
	...
	"github.com/coldzerofear/vgpu-manager/cmd/kubelet-plugin/featuregates"
	...
	configapi "github.com/coldzerofear/vgpu-manager/api/nvidia.com/resource/v1beta1"
)
...
type DeviceState struct {
	...
	nvdevlib          *nvidia.DeviceLib
	checkpointManager checkpointmanager.CheckpointManager
}

func NewDeviceState(ctx context.Context, config *Config) (*DeviceState, error) {
	containerDriverRoot := nvidia.RootPath(config.flags.containerDriverRoot)
	nvdevlib, err := nvidia.NewDeviceLib(containerDriverRoot)
	if err != nil {
		return nil, fmt.Errorf("failed to create device library: %w", err)
	}

	allocatable, err := EnumerateAllPossibleDevices(nvdevlib, config)
	if err != nil {
...
func EnumerateAllPossibleDevices(nvdevlib *nvidia.DeviceLib, config *Config) (AllocatableDevices, error) {
	alldevices := make(AllocatableDevices)

	gms, err := EnumerateGpusAndMigDevices(nvdevlib, config)
	if err != nil {
...
func EnumerateGpusAndMigDevices(nvdevlib *nvidia.DeviceLib, config *Config) (AllocatableDevices, error) {
	if err := nvdevlib.NvmlInit(); err != nil {
		return nil, err
	}
	defer nvdevlib.NvmlShutdown()

	devices := make(AllocatableDevices)
	err := nvdevlib.VisitDevices(func(i int, d nvdev.Device) error {
		gpuInfo, err := nvdevlib.GetGpuInfo(i, d)
		if err != nil {
		...
		migs, err := nvdevlib.GetMigInfos(gpuInfo)
		if err != nil {
	...
	for _, result := range claim.Status.Allocation.Devices.Results {
		...
		for _, c := range slices.Backward(configs) {
			if slices.Contains(c.Requests, result.Request) {
				if _, ok := c.Config.(*configapi.GpuConfig); ok && device.Type() != nvidia.GpuDeviceType {
					return nil, fmt.Errorf("cannot apply GPU config to request: %v", result.Request)
				}
				if _, ok := c.Config.(*configapi.MigDeviceConfig); ok && device.Type() != nvidia.MigDeviceType {
		...
		for _, result := range results {
			...
			case nvidia.MigDeviceType:
				preparedDevice.Mig = &PreparedMigDevice{
					Info:   s.allocatable[result.Device].MigInfo,
					Device: device,
				}
			}
