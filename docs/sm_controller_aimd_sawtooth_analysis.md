...
#### V2.1:debounce 范围扩大 + 突发对称回退(当前实现)

V2 落地后又发现两个隐患:

1. **soft 突发判定无防抖**:V2 的 `host_index_is_exclusive` 是裸谓词,被 watcher 主循环直接调用 → 外部 Pod 的 util 短暂归 0(NVML 抖动 / 间歇性工作负载)立刻误触发突发 → 挤压本应受保护的外部 Pod
2. **突发拿到的资源无路回退**:up_limits 只在以下两种情况会被清零回 hard_core:① 进程数变多触发 reset;② 启动时初始化。**外部 Pod util 从低变高(不属于"翻转事件")时,我们已经突发到 soft 的 up_limits 没有任何回退通道**,持续挤压外部 Pod

V2.1 的修法(4 件事并做):

| 改动 | 解决 |
|---|---|
| **拆分谓词**:`host_index_is_exclusive_raw()` 裸观测 + `host_index_is_exclusive_debounced()` 带 N 周期对称防抖 + per-cycle memo | soft 突发判定改用 debounced 防误触发;hard_limit jitter init 改用 raw 保持实时响应 |
| **统一 FSM**:auto 路由直接读 debounced 谓词,删 auto 自己的 FSM。**整个 vgpu-manager 对"独占"的判断只有一个真相源** | 行为一致性;消除 auto/watcher 两套 FSM 漂移可能 |
| **失独占 → 强制 reset**(选项 ①):debounced 谓词从 true → false 翻转的瞬间,设置 `g_lost_exclusivity_pending` 标志,watcher else 分支顶部消费此标志,把 up_limits 强制回到 hard_core(归还突发拿到的份额) | 翻转事件型的快速响应 |
| **对称弹性升降**(选项 ③):原本只在"看到空闲就涨"的周期性调整里,补一个对称分支"看到压力就降",**严格不破 hard_core 下界** | 外部 Pod 负载渐变型变化的持续追踪 |
| **进程加入回退判决** 从 `sys_process_num` 改为 `external_process_num`(新加字段) | 本容器 fork(DataLoader workers / torch.distributed)不再触发自我回退 |

V2.1 谓词初始值取**保守 false**(启动期不允许突发,等观察 N 周期确认独占才放行),代价是单 Pod 启动期 ~400ms 不立即突发,**避免一启动就因为还没观察到外部 Pod 而错误地拿 delta 去挤压可能存在的别人**。

性能开销:per-cycle 在 watcher 主循环顶部清一次 memo 标志,谓词第一次调用时 O(1) 计算,后续同 cycle 调用 memo 命中返回,**~ns 量级,可忽略**。

V2.1 simulator 9/9 场景验证通过(覆盖:NVML 抖动 / lost-reset / 渐变压力 / 内 fork / 驱动线程 / 阈值边界 / hard_core 下界 / 新外部 Pod 加入 / 弹性恢复均衡)。

#### P1 已落地:deadband + MD cooldown(`aimd_controller` 内部)

在 P0/V2 升级后,`aimd_controller()` 函数体本身按方案 ①+② 加固:

| 改动 | 解决 |
|---|---|
| **死区(hysteresis)**:`[up_limit × deadband_ratio/1000, up_limit × eff_ratio/1000]` 内 share 不变 | 消除 V2 在 eff 阈值两侧反复 AI/MD 的锯齿。默认死区 = `[80%, 87.5%]` 即 `7.5%` 宽,**自动随 up_limit 缩放**(小 setpoint 死区窄,大 setpoint 死区宽) |
| **MD cooldown**:MD 触发后 N 个 watcher 周期禁止再 MD,即使 user_current 仍超 eff | NVML 反应延迟 200~400ms 期间不会连续 ÷3 ÷9 ÷27 把 share 砍到 floor。默认 N=3 ≈ 240ms(80ms 周期 × 3),`= 0` 关闭恢复 V2 行为 |
| 死区上下界 clamp(`deadband_low < eff_limit`) | 启动时 `sm_controller_init` clamp + 函数内 defence-in-depth |
| AI 区/死区/MD 区**三态决策**(替换原本二态 `≤ eff` 或 `> eff`) | 函数体多 3 条 int 算术 + 2 条 int 比较,~ns 量级,可忽略 |

cooldown 实现细节:set 时为 `cycles + 1`,decrement 在函数入口,这样"cooldown=N"刚好 gate N 个 post-MD 周期(`MD 后 1..N` 全部 blocked,cycle N+1 free)。**避开"边界少一周期"的 off-by-one 陷阱**。

P1 simulator 8/8 场景验证通过(覆盖:死区稳定 / 边界 user==eff / MD 雪崩防护 / cooldown=0 fallback / 小 setpoint 区间排序 / 配置防御 / cooldown 时间式衰减 / AI 区基本功能)。
...
| 优先级 | 方案 | 主要收益 | 工作量 |
|---|---|---|---|
| **P0**(已落地 V2.1) | **⑤ sys_process_num 自动路由** | 直接解用户报告的"单 Pod 训练慢 1/3" | ⭐⭐ |
| **P1**(已落地) | **① deadband + ② MD cooldown 一起做** | 即使留在 AIMD,锯齿幅度也明显收窄 | ⭐⭐(两个加一起) |
