...
| **跨节点子域对齐(rail 对齐)** | 每 Pod 取节点子集(如 4/8)且节点有多个 NVLink 域时,各 Pod 须选**跨节点对应的同一域** | ⚠️ **需扩展(§7.2),仅子卡分配场景** |
...
- **Kueue 只到节点粒度**:它注入 `NodeSelector(hostname)` 选节点,**对"节点内是哪个子域"完全无感**——k8s 调度本就是节点粒度,子卡域选择天然是设备分配器(vgpu-manager)的职责。Kueue 无解。
...
2. **把所选 ordinal 落到 Pod 注解(allocator)**:gang Pod 分配到某分量后,把其 ordinal 写进注解,如 `nvidia.com/gang-link-ordinal: "0"`。**关键**:这样后续兄弟**无需重建别人节点的拓扑**,直接读这个整数即可——避免了"node2 去理解 node1 的 union-find"这种做不到的事。

3. **跨节点 anchor 解析(allocator)**:调度某 gang Pod 时按优先级:
   - **同节点**有已分配兄弟 → 用现有本地 root anchor(最精确,节点内连通)。
   - 否则**全集群**查同 gang 兄弟:用 `podLister.ListByIndexValue`(已确认返回全集群 vGPU Pod 扁平列表,**不只本节点**)找到任一已分配兄弟,读其 `gang-link-ordinal` → 目标 ordinal → 在**本节点**把候选窗口过滤到该 ordinal 的分量。
   - **首 Pod**(无兄弟):选最优分量,其 ordinal 即成为全组目标,写注解。
...
**落地定位**:本质是把已落地的跨 Pod anchor(阶段一)**从同节点扩展到跨节点**,复用 `podLister`(全集群)+ Pod 注解,**不引入新 CRD**(契合既定"不再加 CRD 解析"的取向),改动适中。
...
3. **常见场景(整机整卡)零额外开发**;两个边界都源于"Kueue 节点级计数看不到节点内 NVLink 域":§7.1 装不下(用整机整域 + 非 strict link + 整卡申请规避),§7.2 子卡分配时的跨节点子域对齐(需把跨 Pod anchor 从同节点扩展到**跨节点稳定 ordinal**,复用 podLister + Pod 注解、不加 CRD;同构集群可行,严格 rail 对齐待发布 GPU-NIC 拓扑)。整机整卡可同时绕过这两个边界。
