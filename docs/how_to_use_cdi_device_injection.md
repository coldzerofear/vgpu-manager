...
> 驱动安装在宿主机的常见场景，使用默认值 `/` 即可，**无需额外挂载**。
>
> GPU-Operator 驱动容器场景（驱动不在宿主机上）：`nvcdi` 需要在**插件容器内**读取驱动根目录，
> 因此除了设置 `--driver-root`（容器内读取路径）/`--target-driver-root`（宿主机路径），还必须把
> **宿主机驱动根目录挂载进插件容器**。Helm chart 会在 `driverRoot` 设为非 `/` 值时自动添加该挂载
> （`hostPath=targetDriverRoot` → 容器 `driverRoot`），并保证 flag 与挂载路径一致；`deploy/` 原始
> YAML 中已提供对应的注释示例，按需取消注释。
...
devicePlugin:
  devicePlugin:
    commands:
      ...
      # 可选：自定义 CDI 选项
      cdiRoot: "/var/run/cdi"
      # GPU-Operator 驱动容器场景（chart 会据此自动挂载宿主机驱动根目录进插件容器）：
      # driverRoot: "/driver-root"               # 插件容器内读取路径
      # targetDriverRoot: "/run/nvidia/driver"   # 宿主机驱动根目录
```
