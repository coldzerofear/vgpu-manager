...
| `--driver-root` | `/` | 驱动根目录。宿主机同名路径会挂载进插件容器，因此它既是容器内读取路径、也是写入规范的宿主机路径 |
| `--dev-root` | （同 driver-root） | 设备节点根目录，同样按相同路径挂载 |
...
devicePlugin:
  devicePlugin:
    commands:
      ...
      # 可选：自定义 CDI 选项
      cdiRoot: "/var/run/cdi"
      # GPU-Operator 驱动容器场景（chart 会据此把同名路径挂载进 plugin 与 monitor 容器）：
...
- 若开启 CDI 但容器起不来 / 找不到驱动库，先确认：
  ...
  2. 容器运行时是否启用了 CDI；
  3. `--driver-root` 是否与实际驱动位置一致，且该路径已挂载进插件容器。
