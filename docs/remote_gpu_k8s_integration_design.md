...
| D27 | 会话生命周期与回收（v2.3，用户提出） | **会话合法性的唯一事实来源 = token 出现在 claim 当前的 `session-*` 注解里**（inject 在 EnsureSession 之前先写注解、在 NodeUnprepare 时删注解）。三层回收：(1) **显式释放**：inject 收到 NodeUnprepare 后 GET claim，若 claim 已删或 `ReservedFor` 里没有仍在运行的 Pod（本节点的 Pod 由 kubelet 保证已结束；其他节点的按 phase 判断；非 Pod 消费者视为存活）→ 先删注解、再对每个 agent 调 `ReleaseSessions(claim_uid, tokens)`（best effort）；仍有存活消费者（跨节点共享的独立 claim）则不动。(2) **事件清扫**：agent 的 claim informer 保留 `session-*` 注解，每次 Add/Update 按"注解 token 集合"清扫；allocation 为空或删除中 → 集合为空。(3) **周期兜底**：每分钟按缓存里的 claim 做同样比对。**版本守卫**：会话标记文件记录物化时 claim 的 resourceVersion，只有"事件版本 ≥ 标记版本"的清扫才生效，避免独立 claim 复用时滞后的"已释放"事件误删新 Pod 刚建的会话；EnsureSession 只接受 token 已登记在 claim 上的请求（缓存里没有则直读 apiserver），因此标记版本必然 ≥ 写注解的版本。agent 重启期间发生的复用由 (3) 的注解比对兜住（旧 token 不在注解里即回收），旧 token 不再是有效钥匙。**token 按分配作用域**：inject 在 claim 上记录 `allocation-id`（分配结果的摘要），分配变化（独立 claim 释放后再分配）时旧 token 一律作废并重新签发，agent 的 `ClaimSessions` 也只承认与当前分配匹配的 token，因此 ReservedFor 摘除与 NodeUnprepare 的先后顺序不再影响正确性。**跨节点写注解用乐观锁**：assignTokens / cleanTokens 的 merge patch 带 `metadata.resourceVersion`，冲突则重读 claim 重新决策（client-go RetryOnConflict），避免一个节点按旧快照删掉另一节点刚签发的 token；inject 插件启用 `Serialize(true)` 串行本节点回调，EnsureSession 单次超时 5s |
| D28 | agent 接口鉴权与 client 制品自举（v2.3，用户提出） | **token 即凭证**：`EnsureSession` / `ReleaseSessions` / `FetchClientBundle` 都要求调用方持有已登记在 claim 当前分配上的 session token；未登记 → `PermissionDenied`，claim 不存在 → `NotFound`，`ReleaseSessions` 不再接受"只给 claim UID 释放全部"（UID 不是秘密；claim 已删的会话由 agent 自己的 claim watch 清扫，inject 用 prepared 缓存里的 token 补一次释放）。边界：token 写在 claim 注解上，集群内有 claim 读权限者可见，因此这层挡的是网络访问者而非集群内读者（EnsureSession 幂等、重放无害），跨信任域仍需 D5。**client 制品自举**：lupine-server 在 RPC 端口以 HTTP/1.x 提供 `/.well-known/lupine/client/v1/<os>/<arch>`（zip：manifest.json + 三个 shim；响应头 `etag`=`"sha256:<hex>"`、`content-digest`），agent 探测时 HEAD 取 etag，随 `ServerInfo` / `EnsureSession` 回包一起下发，并以 `FetchClientBundle` 流式 RPC 代理下载（同一 token 校验；grpc 与 unix 监听都可用，因此 server 只绑高性能网卡、pod 网络不通时消费节点仍能取到 client）。inject 的 prepare 顺序改为 **先 EnsureSession 拿 etag，再定制品**：版本目录由 bundle 安装的记录 `.etag`，与 server 当前 etag 一致直接用，不一致就重新拉取替换（旧目录改名 `.stale-*` 保留给仍在用的 pod，不参与选择）；init 容器铺的目录没有 `.etag`，按运维背书原样使用；没有可用目录时从 floor 设备的 agent 下载，校验 etag/content-digest/manifest 每文件 sha256 后以 floor 版本号落盘。NRI 模式 prepare 时还没有会话 token，用一个 claim 级 `claim-prepare` token（同样写注解、同样随 Unprepare 清理、从不物化会话）作下载凭证。制品 etag 已知时向 pod 注入 `LUPINE_CLIENT_ETAG` / `LUPINE_CLIENT_PLATFORM`，让 server 用自己的 426 校验兜底 client/server 构建不一致。init 容器降级为可选预铺 |
