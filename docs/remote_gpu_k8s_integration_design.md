...
| D26 | lupine-server 状态的唯一来源（v2.3，用户提出） | **remote-agent 是 lupine-server 状态的唯一来源**：agent 每 5s 探测 server（HTTP GET 读 `x-lupine-cuda-version`），`ServerInfo` gRPC 暴露 `listening` / `cuda_driver_version` / `endpoint`；dra-server 的发布器与 inject **不再直接访问 lupine-server**，只需 agent 地址。`endpoint` 的来源按优先级：`--advertise-server-endpoint`（运维指定，不探测）> 探测地址 host 可路由则原样 > 探测地址是回环时在本机地址中发现（候选序 = 节点 InternalIP > 物理网卡 > 虚拟网卡，逐个用同一 GET 验证，命中后粘住直到失效；lupine 绑 `INADDR_ANY`，所以不能靠监听表反查 IP）。dra-server 侧 `--remote-server-endpoint` 留空 host 即采用 agent 报告值（版本或地址变化都重发 slice），显式 host 则钉死不被覆盖；`--remote-agent-local-endpoint` 允许本机经 `unix:///etc/vgpu-manager/agent.sock` 访问 agent（agent `--listen-server-endpoint` 支持逗号分隔的 grpc:// + unix:// 多监听；unix 只服务同节点、不发布）。为 D25（DNS）预留：agent 已持有对外 endpoint，后续可由它维护记录 |
