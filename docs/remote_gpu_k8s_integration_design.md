...
| D23 | 统一设备模型（v2.0，用户提出） | **远程不是新池，是既有设备的一个发布属性**。每台 GPU 节点仍只有一个 pool（= 节点名），gate 开时：设备属性叠加 `accessMode=remote`、`endpoint`、`netZone`（驱动默认域非限定名），pool 用 nodeSelector（`kubernetes.io/hostname=本节点` OR **运维自定义的 `--remote-node-selector` label selector 表达式**，标准语法：`k=v,k2 in (a,b),!k3`，term 内 AND、两 term 之间 OR；不再有固定的 net-zone 标签约定，`netZone` 属性退化为可选信息），helper 传 `ReconcilePoolWithName(节点名)`（上游为"节点拥有、集群可见"专设的选项）。gate 关 → `accessMode=local` + `nodeName`，与今天全等。**v2.1 修订（用户提出）：gate 开的节点只发布、不做本地分配**——该节点上的每个 NodePrepare 都委托给远程 Injector，pod 即使落在 GPU 节点也经 lupine（loopback）消费。理由：一个 claim/pod 可能同时含本节点与其他节点的设备，本地路径（LD_PRELOAD 真驱动）与远程路径（lupine shim + `LUPINE_DISABLE_LOCAL`）在一个容器内互斥，无法按设备拆分注入。代价：同节点消费多一跳 TCP 环回；收益：路径唯一、无混合冲突。**GPU 节点因此也必须铺 client 制品目录**（chart 注意）。**一台设备只有一个 accessMode 值**：class 只能选"未放开节点的设备"或"已放开节点的设备"，表达不了"同一设备但只本地"——只本地靠节点不开 gate 或 pod nodeAffinity。本地优先打分为后续专项（DRA 插件当前不打分） |
