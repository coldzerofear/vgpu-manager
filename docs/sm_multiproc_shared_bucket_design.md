...
```c
limit = clamp(sample_interval_ns * 3, 270ms, 5s)
```

- `sample_interval_ns` 由采样主发布（它自己两次发布之间的实测间隔，ABI v3 新增字段）。
- **下限 270ms**：采样主很快时不至于过于敏感。
- **上限 5s**：防止一次异常间隔（机器挂起、时钟异常、被调度器饿了几秒）把阈值永久顶到再也不会触发的地方。
- 换手后第一次发布**不计间隔**（跨越接管的间隔反映的是接管本身，不是节拍）。

于是：健康但慢的采样主**被信任**；真正停摆的采样主**仍然会在它自己的 3 个节拍内被发现**。用例 [6] 锁死这一点，并显式断言"固定阈值会拒绝 440ms 的采样主、自适应阈值接受"。
