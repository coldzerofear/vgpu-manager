...
3. **阶段 2（本地批量取令牌）是否要做**：仍**仅在 profiling 证明 cacheline 弹跳是真开销后**才做（§4.12）。

### 12.3 我已自行决定的（理由在文档内，不同意直接打）

内存序选择（§4.10）、`ofd_fcntl` 复用与回退（§4.4.2）、就地重建 vs unlink/rename（§4.5.4）、冻结区字段集与永久 ABI 约束（§4.1）、Go 侧"只跳过不重建"的不对称授权（§10.3）、实施阶段拆分（§8）。
