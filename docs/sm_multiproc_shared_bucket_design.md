...
- **不做每进程独立限额**（即"A、B 各限 50%、合计 100%"那种语义）。我们是**容器聚合**语义，见 §3.3 的对比。
...
| 维度 | 锁串行化方案 | Event + 协调标记方案 | **本方案（共享桶 + CAS）** |
|---|---|---|---|
| 限额语义 | 聚合 | **每进程**（合计 = N×限额） | 聚合 |
| 每次发射开销 | 锁/解锁，争用陷内核 ~μs | 无（但要 cuEvent 同步） | **1 条 CAS（≈现状）** |
| 并行性 | **串行化，吞吐塌** | 并行 | 全并行 |
| 崩溃语义 | 持锁猝死 → **全容器死锁** | 标记残留 → 可自愈 | 无临界区 → **自愈** |
| 抗空转（配额回收） | 无 | 靠"我睡了、你们上"式广播主动回收 | **共享桶天然回收**（A 不取、B 自然取走） |

Event + 协调标记方案里的协调字段：它**不是锁、不串行化发射**，而是**抗空转**——每进程独立限额下，A 睡时 GPU 会空，于是广播"我睡了、你们上"让 B 错峰填充。**在聚合语义 + 共享桶下，这个协调是白送的**：A 不消费令牌，B 自然就消费走了，无需协调字段、无需扫描兄弟。故本方案**不移植**这套机制。
...
**这与被否决的锁串行化方案有本质区别**（§3.3）：

| | 锁串行化方案的共享内存互斥锁 | 本设计的初始化文件锁 |
|---|---|---|
| 位置 | **每次 kernel 发射**（热路径） | 进程初始化，**一次** |
| 持锁者猝死 | 锁永久残留 → **全容器死锁** | **内核自动释放** → 无残留 |
| 对发射的影响 | 串行化，吞吐塌 | **零**（热路径不碰它） |

即"持锁猝死 → 死锁"这条否决锁串行化方案的理由，**对内核文件锁不成立**——进程死亡时内核无条件回收其文件锁。
...
本方案**不引入任何用户态 mutex**（热路径全是 CAS + 选举；初始化用内核文件锁），因此**无需**动 [`loader_child_after_fork`](../library/src/loader.c#L2635) 的 mutex 重init 列表，规避了"父持锁 fork → 子死锁"这一整类隐患。这是相对锁串行化方案的结构性优势。
