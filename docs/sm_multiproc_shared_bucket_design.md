...
```c
/* 容器内路径 /tmp/.sm_bucket；宿主实际目录由 device plugin 按
 * <host_manager_dir>/<pod-uid>_<cont-name>/sm_bucket 挂入（见 §4.5）。 */
#define SM_BUCKET_DIR       "/.sm_bucket"
#define SM_BUCKET_FILE_PATH (TMP_DIR SM_BUCKET_DIR "/sm_bucket.config")

#define SM_BUCKET_MAGIC          0x53_4D_42_4BU   /* "SMBK" */
#define SM_BUCKET_LAYOUT_VERSION 1U               /* 改结构体必须 +1，见 §4.5 */
...
typedef struct {
  ...
  uint32_t magic;             /* SM_BUCKET_MAGIC                          */
  uint32_t layout_version;    /* SM_BUCKET_LAYOUT_VERSION                 */
  uint32_t region_size;       /* sizeof(sm_bucket_region_t)               */
  uint32_t device_count;      /* MAX_DEVICE_COUNT                         */
  uint8_t  _pad[CACHELINE_SIZE - 16];
  sm_bucket_dev_t devices[MAX_DEVICE_COUNT];
} sm_bucket_region_t;

_Static_assert(sizeof(sm_bucket_dev_t) == CACHELINE_SIZE, "sm_bucket_dev_t must be one cacheline");
_Static_assert(offsetof(sm_bucket_region_t, devices) == CACHELINE_SIZE, "region header must be one cacheline");
```
...
### 4.5 陈旧状态：不做 generation，改做布局守卫

> **本节推翻了初稿的设计。** 初稿引入 `generation`（容器实例代号）来清理"上一世"残留，并把来源列为待定。核查宿主侧挂载后，该需求基本不成立，而 `generation` 恰好防不住真实存在的那个危险。

#### 4.5.1 为什么不需要 generation

共享目录**不是**容器自己的 `/tmp`，而是 device plugin 从宿主机挂进去的（[vnum_plugin.go#L831](../pkg/deviceplugin/vgpu/vnum_plugin.go#L831)）：
...
**宿主目录已按 `<pod-uid>_<容器名>` 分好**。据此逐场景核对：

| 场景 | 宿主目录 | 残留 | 该不该重置 |
|---|---|---|---|
| Pod 重建 | pod UID 变 → **全新空目录** | **不可能有** | 无所谓（本就是新区） |
| 容器在 Pod 内重启 | 同 UID 同名 → 同目录 | 有 | 可以，但见下 |
| 容器内业务进程重启 | 同目录 | 有 | **不该**（兄弟进程还活着） |

即"跨 Pod 继承上一世状态"**物理上不可能发生**——那正是 generation 的主要假想敌。

而"容器在 Pod 内重启"残留下来的是什么？`share` / `cur_cuda_cores` / `up_limit` / `is_cnt` / `avg_sys_free` **全是负反馈量**：控制器几个周期（~百毫秒级）就把它们拉回收敛值，起始值是垃圾也无妨；`total_cuda_cores` 是设备几何，同一次分配下**残留值本来就是对的**。

> **关键区别（初稿的判断失误）**：我把令牌桶当成了 vmem 那样的**账本**。账本的陈旧条目永不自愈、必须清理；令牌桶是**自校正的反馈量**，重置漏做的代价 ≈ 几个控制周期。这个不对称决定了本节不值得上机制。

#### 4.5.2 真正的危险：库版本升级导致布局错位

宿主目录跨容器重启存活，而 `.so` 是**按版本挂载**的（[vnum_plugin.go#L492](../pkg/deviceplugin/vgpu/vnum_plugin.go#L492)：`libvgpu-control.so.<version>`）。于是存在这条真实路径：

> 升级 library → 容器在 Pod 内重启 → **新库映射到老结构体的字节** → 字段错位 → 令牌桶读出垃圾。

这个危险 `generation` **防不住**（它只问"是不是新一世"，不问"结构体长得一样吗"）。防它的是**布局守卫**：

```c
/* attach 后，任何字段读取之前： */
if (region->magic          != SM_BUCKET_MAGIC          ||
    region->layout_version != SM_BUCKET_LAYOUT_VERSION ||
    region->region_size    != sizeof(sm_bucket_region_t) ||
...
- 全 0 的新文件天然 `magic != MAGIC` → 走首建初始化，**建区与升级重整是同一条路径**，无需额外分支。
- **改结构体必须 bump `SM_BUCKET_LAYOUT_VERSION`** —— 这是本设计对未来维护者的硬约束，应写进结构体上方注释。

#### 4.5.3 可选：ns st_ino 作为 best-effort 实例标识

若仍想在"容器在 Pod 内重启"时主动清一次（纯粹为了干净，非正确性所需），可复用现成实现——[`device_pid_in_same_container()`](../library/src/util.c#L546) 已经在用 `{mnt, cgroup}` 两个 namespace 的 `st_ino` 组合判定同容器：

```c
const char *ns_types[] = {"mnt", "cgroup", NULL};   // util.c:549 现成
```

语义正好对齐：容器在 Pod 内重启 → kubelet 建新容器 → mnt/cgroup ns 重建 → `st_ino` 变 → 触发重置（正确）；容器内业务进程重启 → ns 不变 → 不重置（正确，兄弟进程还在）。

**但必须标注为 best-effort**：nsfs 的 inode 号由内核 IDA 分配，**释放后会被复用**（且优先分配最小空闲 ID），容器频繁起落时复用完全可能；mnt+cgroup 双字段只降低碰撞、不消除。所以"`st_ino` 必然变化"的说法**不成立**。

按 §4.5.1，碰撞的后果仅是少重置一次 → 几个控制周期的收敛延迟 → **可以接受**。故此项列为**可选优化，非正确性依赖**；正确性由 §4.5.2 的布局守卫独立保证。
...
- **[中] 库升级后的布局错位**（§4.5.2）：宿主目录按 `<pod-uid>_<cont-name>` 跨容器重启存活，而 `.so` 按版本挂载 → 新库可能映射到老结构体。靠 `magic`/`layout_version`/`region_size` 守卫；**改结构体必须 bump `SM_BUCKET_LAYOUT_VERSION`**，需在 review 中把关。
- **[低] ns st_ino 复用**（§4.5.3）：可选的实例重置是 best-effort（nsfs inum 由 IDA 分配、会复用）。碰撞后果仅为少重置一次 → 几个控制周期收敛延迟，可接受。
...
- **不做 generation**（§4.5）：宿主目录已按 `<pod-uid>_<cont-name>` 隔离 → 跨 Pod 残留物理上不可能；容器内残留全是**自校正的反馈量**（不是 vmem 那样的账本）。改用 `magic`/`layout_version`/`region_size` **布局守卫**，防的是 generation 防不住的真实危险：**库升级后新库读老结构体**。
