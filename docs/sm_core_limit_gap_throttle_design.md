...
- **✅ 已落地 — AIMD watcher 控制器**(详见 [sm_controller_aimd.md](sm_controller_aimd.md)):与 GAP 路径**正交、互补**。曾因单 Pod 锯齿问题移除,后因 delta 的 sm² 增量缺陷在大 SM 卡上限不住核心而恢复(HAMi-core #274 同源,见该文档沿革节)。`CUDA_SM_CONTROLLER=aimd` 启用,默认 `delta`(stock)。
