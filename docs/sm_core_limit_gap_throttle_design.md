...
- **❌ 已移除 — AIMD watcher 控制器**(历史记录见 [sm_controller_aimd.md](sm_controller_aimd.md)):曾作为 delta 之外的可选控制器,因实测利用率控制效果不达预期而删除;`CUDA_SM_CONTROLLER`/`CUDA_SM_AIMD_*` 已失效。GAP 路径不受影响,`delta` 是唯一控制器。
