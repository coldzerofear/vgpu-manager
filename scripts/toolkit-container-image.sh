...
# Handle format: Go pseudo-version format vX.Y.Z-0.yyyyMMddHHmmss-commitSHA
if [[ "${TOOLKIT_VERSION}" =~ ^v[0-9]+\.[0-9]+\.[0-9]+-[0-9]+\.[0-9]{14}-([a-f0-9]{12,})$ ]]; then
    TOOLKIT_VERSION_SHA="${BASH_REMATCH[1]}"
    SHORT_SHA="${TOOLKIT_VERSION_SHA:0:8}"
    IMAGE_URL="ghcr.io/nvidia/container-toolkit:${SHORT_SHA}"
# Handle format vX.Y.Z-time-commit (original)
elif [[ "${TOOLKIT_VERSION}" =~ ^v[0-9]+\.[0-9]+\.[0-9]+-[0-9]{14}-([a-f0-9]{12})$ ]]; then
    TOOLKIT_VERSION_SHA="${BASH_REMATCH[1]}"
    SHORT_SHA="${TOOLKIT_VERSION_SHA:0:8}"
    IMAGE_URL="ghcr.io/nvidia/container-toolkit:${SHORT_SHA}"
