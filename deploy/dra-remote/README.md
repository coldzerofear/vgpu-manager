...
| **服务端 endpoint** | `remote-server.yaml` `LUPINE_PORT` + agent 的 `REMOTE_SERVER_ENDPOINT`（探测地址，默认 127.0.0.1）；可选 `ADVERTISE_SERVER_ENDPOINT`（对外地址，运维指定）；`dra-server.yaml` `REMOTE_SERVER_ENDPOINT`（发布值，host 留空 = 采用 agent 报告的） | `:14833` | URL 形态（可带 `https://`、域名、路径前缀，为将来 DNS + 网关路由预留）。**改端口只改 remote-server 两处**，dra-server 从 agent 拿到完整 endpoint；dra-server 显式写了 host 则原样发布、agent 不覆盖。发布为设备属性 `serverEndpoint` |
| **agent endpoint** | `remote-server.yaml` `LISTEN_SERVER_ENDPOINT`（逗号分隔可多个：`grpc://:14834` 与 `unix:///etc/vgpu-manager/agent.sock`）、`dra-server.yaml` `REMOTE_AGENT_ENDPOINT`（发布值）与可选 `REMOTE_AGENT_LOCAL_ENDPOINT`（本机访问 agent 的地址，可为 unix 套接字） | `:14834`（发布 host 留空 = 同服务端 host） | TCP 端口两处必须一致（hostNetwork）；发布为设备属性 `agentEndpoint`，inject 按它调 EnsureSession（dra-inject 无需再配端口）。unix 套接字只供同节点组件，不会被发布 |
